...
export class MailsEffect {
   ...
   */
  @Effect()
  updateMail: Observable<MailsActions.MailsActionsAll> = this.actions.pipe(
  ...
  checkCurrentMail: Observable<Action> = this.actions.pipe(
    ...
    map(([action, state]) => {
      if (
        this.routerState.params.mailId &&
        !state.entities[this.routerState.params.mailId]
      ) {
  ...
  setFolderOnSelectedMails: Observable<
    MailsActions.MailsActionsAll
  > = this.actions.pipe(
    ofType<MailsActions.SetFolderOnSelectedMails>(
      MailsActions.SET_FOLDER_ON_SELECTED_MAILS
    ),
    withLatestFrom(this.store.pipe(select(getMailsState))),
    map(([action, state]) => {
  ...
  addLabelOnSelectedMails: Observable<
    MailsActions.MailsActionsAll
  > = this.actions.pipe(
    ofType<MailsActions.AddLabelOnSelectedMails>(
      MailsActions.ADD_LABEL_ON_SELECTED_MAILS
    ),
    withLatestFrom(this.store.pipe(select(getMailsState))),
    map(([action, state]) => {
      const entities = { ...state.entities };
