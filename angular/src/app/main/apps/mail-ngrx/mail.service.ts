...
export class MailNgrxService {
  ...
  getMails(handle): Observable<Mail[]> {
    if (handle.id === 'labelHandle') {
      const labelId = this.labelsArr.find(
        (label) => label.handle === handle.value
      ).id;
      ...
      return this._httpClient.get<Mail[]>(
        'api/mail-mails?' + handle.value + '=true'
      );
    } // folderHandle
    else {
      const folderId = this.foldersArr.find(
        (folder) => folder.handle === handle.value
      ).id;
